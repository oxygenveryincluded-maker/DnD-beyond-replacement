...
<form onsubmit={(e) => { e.preventDefault(); handleSearch(); }} class="relative {compact ? 'w-full' : 'w-full max-w-md'}">
  <input
    ...
    bind:value={query}
    {placeholder}
    class="w-full pl-10 pr-4 py-2.5 text-sm bg-dnd-card border border-dnd-border rounded-lg text-dnd-text placeholder-dnd-text-muted focus:border-dnd-gold focus:ring-1 focus:ring-dnd-gold/30"
  />
